    ...
    fn try_from(value: Token<'a>) -> Result<Self> {
        match value {
            Token::CharacterProgramData(s) => match s {
                x if scpi::parser::mnemonic_compare(b"Cel", x) => Ok(Self::Celsius),
                x if scpi::parser::mnemonic_compare(b"Far", x) => Ok(Self::Fahrenheit),
                x if scpi::parser::mnemonic_compare(b"K", x) => Ok(Self::Kelvin),
