...
use crate::{cmd_nquery, error::Result, option::ScpiEnum, tree::prelude::*};
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, scpi_derive::ScpiEnum)]
#[non_exhaustive]
...
pub trait Trigger<SRC, const SEQ: usize = 1>: Abort + Initiate {
    type Delay;

    fn count(&mut self, count: NumericValue<usize>) -> Result<()>;
    fn get_count(&self) -> usize;

    fn delay(&mut self, delay: NumericValue<Self::Delay>) -> Result<()>;
    fn get_delay(&self) -> Self::Delay;

    /// Set source of trigger
    fn source(&mut self, source: SRC) -> Result<()>;
    fn get_source(&self) -> SRC;
}

pub struct TrigSeqSourceCommand<SRC: ScpiEnum = TriggerSource, const SEQ: usize = 1> {
    _phantom: PhantomData<SRC>,
}

impl<SRC: ScpiEnum, const SEQ: usize> TrigSeqSourceCommand<SRC, SEQ> {
...
impl<D, SRC, const SEQ: usize> Command<D> for TrigSeqSourceCommand<SRC, SEQ>
where
    D: ScpiDevice + Trigger<SRC>,
    SRC: for<'a> TryFrom<Token<'a>, Error = Error> + ResponseData + ScpiEnum,
{
    cmd_nquery!();

    fn event(&self, device: &mut D, _context: &mut Context, mut args: Arguments) -> Result<()> {
        let src: SRC = args.data()?;
        device.source(src)
    }
}
