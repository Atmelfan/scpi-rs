...
use crate::{format::Expression, prelude::{Tokenizer, Arguments, ErrorCode, Token}, error::Result, option::ScpiEnum, command::{Command, CommandTypeMeta}};

use super::{trigger::Trigger, ScpiDevice};

trait MeasurementFunctions: Trigger {
    /// CONFigure
    fn configure(
        &mut self,
    ...
    /// CONFigure?
    fn get_configuation(&self) -> (Option<Presentation>, Option<Fundamental>, Function);
    ...
    fn read(
        ...
        self.abort();
        self.initiate();
        self.fetch(presentation, fundamental, function, source_list)
    }
    ...
    fn meas(
        ...
        self.abort();
        self.configure(presentation, fundamental, function);
        self.read(presentation, fundamental, function, source_list)
    }
...
/// # Presentation layer
/// The purpose of the presentation layer is to handle groups of data and present them in the way
/// the user specifies. This area is still under discussion, and therefore most of the presentation
/// types (for example, histograms, time and frequency domain waveforms, and so on) are not
...
/// The purpose of the fundamental measurement layer is to specify the fundamental signal
...
pub enum Function {
    Dc,
    Ac,
    Frequency { expected: f32, resolution: f32 },
    Period { expected: f32, resolution: f32 },
    Phase { expected: f32, resolution: f32 },
}

struct ConfCommand;

impl<D> Command<D> for ConfCommand where D: ScpiDevice + MeasurementFunctions {
    fn meta(&self) -> CommandTypeMeta {
        CommandTypeMeta::Both
    }

    fn event(&self, device: &mut D, _context: &mut crate::Context, mut args: Arguments) -> Result<()> {
        //
        let arg1 = args.next_optional()?;
        let arg2 = args.next_optional()?;
        match (arg1, arg2) {
            //
            (Some(Token::ExpressionProgramData(..)), Some(Token::ExpressionProgramData(..))) => {
                let arg1 = args.next_optional()?;
                let arg2 = args.next_optional()?;
                
            },
            (Some(Token::ExpressionProgramData(..)), arg1) => {
                let arg2 = args.next_optional()?;
    ...
    fn query(
        ...
        _args: Arguments,
        _response: crate::response::ResponseUnit,
    ) -> Result<()> {
        Err(ErrorCode::UndefinedHeader.into())
    }
}
